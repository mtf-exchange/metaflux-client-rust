...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let endpoint = std::env::var("MTF_ENDPOINT")
        .unwrap_or_else(|_| "https://devnet-gateway.mtf.exchange".into());
    ...
    if std::env::var("MTF_SKIP_REGISTER").is_err() {
        let resp2 = client
            .exchange()
            .register_metaliquidity_operator_typed(&wallet, vault_id, wallet.address(), true, 0)
            .await?;
        println!("register_metaliquidity_operator_typed response:\n{resp2:#}");
    }
    Ok(())
}
